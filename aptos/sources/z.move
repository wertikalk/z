module z_aptos::Resolver {
    ...
    public entry fun initialize_resolver<CoinType>(resolver: &signer, withdrawal_address: address) {
        move_to(resolver, AllEscrows<CoinType> {
    ...
    public entry fun create_escrow<CoinType>(
        ...
        assert!(dbg_hashed_secret == hashed_secret, error::invalid_argument(E_DBG_INVALID_SECRET_TO_HASHED));
        ...
        assert!(!table::contains(&locks.locks, key_hash), error::already_exists(E_ESCROW_ALREADY_EXISTS));
        table::add(&mut locks.locks, key_hash, Escrow<CoinType> { coins, unlock_time_secs });
        locks.total_active_escrows = locks.total_active_escrows + 1;
    }

    /// Release the funds from the `Escrow` using the `secret`.
    public entry fun release_funds<CoinType>(maker: &signer, resolver: address, secret: u256) acquires AllEscrows {
        assert!(exists<AllEscrows<CoinType>>(resolver), error::not_found(E_RESOLVER_ACCOUNT_NOT_INITIALIZED));
        ...
        assert!(table::contains(&locks.locks, key_hash), error::not_found(E_ESCROW_NOT_FOUND));

        let Escrow { coins, unlock_time_secs } = table::remove(&mut locks.locks, key_hash);
        locks.total_active_escrows = locks.total_active_escrows - 1;
    ...
    fun assert_resolver_initialized<CoinType>(resolver: &signer) {
        let resolver_address = signer::address_of(resolver);
        assert!(exists<AllEscrows<CoinType>>(resolver_address), error::not_found(E_RESOLVER_ACCOUNT_NOT_INITIALIZED));
    }
    ...
    public entry fun test_basic_flow(
        aptos_framework: &signer, resolver: &signer, maker: &signer) acquires AllEscrows 
    {
        ...
        //  Maker side
        let secret: u256 = 0x15d87951228b5f5de52a2ca404622c9ebd06d662f0d74395f366c4239abaf67a;
        // debug::print(&secret);
        ...
        create_escrow<AptosCoin>(resolver, maker_addr, hashed_secret, secret, 1000, 1000);
    ...
    #[test_only]
    fun basic_test_setup(aptos_framework: &signer, resolver: &signer): BurnCapability<AptosCoin> {
